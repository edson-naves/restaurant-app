...
## New table renders outside its zone — DEPLOYED, one post-deploy fix also deployed
...
Independent review (CODEX) on 28854f8: no findings at any severity — confirmed scope (1 commit, 4 files over 784d026), `_table_map_positions()`'s radius formula never goes negative, no `db.commit()`/`db.add()` anywhere near the display computation, Arrange's real-position branch untouched, the lazy import isn't circular, the corrected test actually asserts the fixed (not the old buggy) value, the end-to-end test creates the table through the real route rather than inserting a row directly, and zero diff anywhere in `_table_map_positions`, `_add_tables`, `_free_cells`, or the Arrange save endpoints. Verdict: APPROVED. Integrated and pushed on explicit authorization ("pode publicar") — fast-forwarded local `main` and pushed to `origin/main` at `28854f8`, confirmed via `git fetch` + `/healthz`.

Also deployed in the same push: an admin-only status legend added to the Floor Plan Builder map (`web/templates/admin_tables.html`, commit `47f8392`) — a user question ("why are these chip borders coloured?") revealed that page had a legend for zone colours but none for table status (free/occupied/ready to pay), the actual meaning of each chip's coloured ring. Cosmetic only, no logic change; verified interactively (one table per status) that the new legend's colours match each chip's ring exactly.

Post-deploy fix — many-unplaced-tables-in-one-zone overcrowding (real user report against the live deploy, right after the above went out): `_table_map_positions()`'s fallback used to spread a zone's never-placed tables around a **ring** centred on the zone. A ring only uses the zone's perimeter — once a zone accumulates more than a handful of never-arranged tables (reported case: adding one 20-seat table to a "Window" zone that already held over a dozen others with no saved position), the ring runs out of room and the rendered cards overlap heavily; worse, the ring re-angles *every* table in the zone whenever the unplaced count changes by even one, so a single addition can make a dozen unrelated cards appear to "jump" at once — exactly what the user described as "created a truckload of nonsense tables" (nothing new was actually created beyond their one table; the others had been sitting unplaced in that zone since before this whole port, and the ring redistribution made them all visually reshuffle together). Root-cause fix: replaced the ring with a **grid** that fills the zone's actual rectangle (cell size approximating the rendered 132px map card, converted to per-mille against the plane's own 1360×1120px CSS floor), row-major by table id rank. Verified two ways: (1) a real browser session recreating the reported shape (17 unplaced tables, one zone) — every table lands in its own distinct cell inside the zone, zero overlap, confirmed with a screenshot; (2) the automated suite got a new test seeding 12 unplaced tables in one zone and asserting they land on at least two distinct rows with twelve distinct positions (a crowded ring would still give distinct-but-cramped points; this proves the 2-D grid structure itself, not just distinctness). A zone genuinely too small for its table count can still overlap some — an inherent limit of any fallback that never resizes the zone itself — but far less than the ring did, and the long-standing fix is unchanged: drag them apart once, and a real saved position overrides this fallback entirely from then on.

This slice (both the original zone-placement fix and both follow-ups above) is DEPLOYED — `origin/main` confirmed at each pushed SHA via `git fetch` + `/healthz` at push time. The grid-rewrite fix itself is implemented and self-tested but **not yet independently reviewed** as of this writing — next step is the same review-then-authorize cycle as every other slice.
