# Review Handoff #3 — Deep-Review Blockers Resolved (Stages 1/2a/2b)

**For:** external reviewer (ChatGPT). **Stage 2c is NOT started** (as instructed).
Covers only the corrections in `CLAUDE_HANDOFF2_DEEP_REVIEW_FEEDBACK.md`
(findings #1–#20), applied additively before any wiring into live money routes.

## Commit range
- **main (pre-remediation) SHA:** `07ee4f1af26aa383b7de06377f18caa98b153e4b`
- **Prior handoff #2 HEAD:** `a6addbe059660b478cc65d92a7d30ab9892df8a2`
- **HEAD (this handoff):** `0621b6ae5207ef5f9bc0081cb5caa8cfb188c94f`
- Branch: `fix/p0-security-and-payments`
- **Commits added this round (a6addbe..HEAD):**
  - `85dbbe2` A — real Postgres upgrade migration for the payment schema
  - `68b5f84` B — refund idempotency fingerprint + charge/payment/provider linkage
  - `be115ee` C — external-processor uncertainty is never assumed away
  - `3a4e552` D — processor-confirmed currency/amount as write-once evidence
  - `0621b6a` E — enforceable capabilities + audited reconciliation authority

## `git status --short`
No uncommitted **source** changes. Untracked, non-source (handoffs + business
docs/decks generated outside the app): `REVIEW_HANDOFF*.md`,
`COMMERCIAL_STRATEGY_90_DAYS*.md`, `MISE_*.pptx`, `.codex_pptx_build/`, `.github/`,
`restaurant-app-review.zip`.

## Finding-by-finding
| # | Sev | Finding | Status | Where |
|---|---|---|---|---|
| 1 | CRIT | Existing PG DBs get the new UNIQUE constraints via migration | ✅ | `migrate._migrate_payment_hardening` |
| 2 | CRIT | payment_attempt fully migrated (width/default/backfill/retire/preserve) | ✅ | `migrate.py` (WIDENED_COLUMNS + hardening) |
| 3 | CRIT | Refund idempotency rejects wrong-intent reuse | ✅ | `refund_attempts.refund_intent_fingerprint` |
| 4 | CRIT | Charge transport ambiguity → reconciliation, not FAILED | ✅ | `square.SquareTransportError`; `SquareTerminalProvider.charge` |
| 5 | — | Currency is per-operation, not global | ✅ | `square.create_checkout/refund(currency_code=)` |
| 6 | — | processor_currency from processor evidence | ✅ | `square.completed_payment_evidence` |
| 7 | CRIT | Refund ↔ charge ↔ provider linkage validated | ✅ | `refund_attempts._resolve_charge_attempt` |
| 8 | HIGH | processor amount/currency write-once | ✅ | `payment_attempts.transition` guards |
| 9 | HIGH | Poll error classification (transient vs definitive) | ✅ | `SquareTerminalProvider.poll` |
| 10 | HIGH | Capabilities are enforceable at registration | ✅ | `payment_providers.register/_validate_capabilities` |
| 11 | HIGH | cancel() validates returned status | ✅ | `SquareTerminalProvider.cancel` |
| 12 | HIGH | OperationalError not blanket-mapped to conflict | ✅ | `payment_attempts.is_lock_conflict` |
| 13 | HIGH | Reconciliation needs role/actor/audit/evidence | ✅ | `payment_attempts.resolve_reconciliation` |
| 14 | — | Migration-UPGRADE test (not just fresh schema) | ✅ | `tests/test_pg_migration.py` |
| 15 | — | Duplicate checkout-id rejected test | ✅ | `tests/test_pg_migration.py` |
| 16 | — | Concurrent refund idempotency test | ✅ | `tests/test_pg_concurrency.py` |
| 17 | — | Pre-tip / tip / total semantics defined | ✅ | `ChargeResult` docstring; poll base=total-tip |
| 18 | MED | Duplicate registry key rejected | ✅ | `payment_providers.register(override=)` |
| 19 | MED | Docs tone down "any provider, no core work" | ✅ | `payment_providers` module docstring |
| 20 | — | (Stage-1 Square doc/validation) — already aligned in handoff #2 | ✅ | `config.py` |

## Exact schema migration operations (`_migrate_payment_hardening`, both engines)
1. **Provider backfill** — `UPDATE payment_attempt SET provider='square_terminal' WHERE provider='square'` (canonical key before constraints).
2. **Retire `provider_refund_id`** — if all-NULL: `ALTER TABLE payment_attempt DROP COLUMN [IF EXISTS] provider_refund_id`; if any non-NULL: **kept + reported** (never silent data loss).
3. **Widen** — `payment_attempt.provider` VARCHAR(20)→VARCHAR(30) (WIDENED_COLUMNS).
4. **Add columns** (ADDED_COLUMNS, guarded) — `intent_fingerprint`, `processor_amount_cents`, `processor_currency`, `reconciled_at`, `reconciled_by`, `reconciliation_note`; `refund_attempt.intent_fingerprint`.
5. **Provider-scoped UNIQUE** — for each of `uq_attempt_provider_payment (provider, provider_payment_id)` and `uq_attempt_provider_checkout (provider, provider_checkout_id)`: skip if present; else **duplicate-scan** (NULLs excluded); **fail closed** with `MigrationError` + the conflicting group if duplicates exist; else `ALTER TABLE … ADD CONSTRAINT …` (PG) / `CREATE UNIQUE INDEX IF NOT EXISTS` (SQLite).

Migration failures are fatal under `strict=True` (production); `main.py` runs `migrate.run(strict=is_production())`.

## PostgreSQL schema introspection (fresh `create_all`)
```
payment_attempt — UNIQUE constraints:
  uq_attempt_idempotency_key    (idempotency_key)
  uq_attempt_payment            (payment_id)
  uq_attempt_provider_checkout  (provider, provider_checkout_id)
  uq_attempt_provider_payment   (provider, provider_payment_id)
  FK: order_id, payment_id, seat_id, staff_id   CHECK: ck_attempt_total_nonneg
  indexes: …_pkey, the 4 uniques, ix_attempt_status, ix_attempt_order

refund_attempt — UNIQUE constraints:
  uq_refund_idempotency_key     (idempotency_key)
  uq_refund_provider_refund     (provider, provider_refund_id)
  FK: charge_attempt_id, payment_id, refund_id, staff_id   CHECK: ck_refund_amount_pos
  indexes: …_pkey, the 2 uniques, ix_refund_attempt_status, ix_refund_attempt_payment
```

## Payment idempotency semantics
Key persisted before contacting the processor and forwarded verbatim to Square.
`create_attempt(key=K)`: existing-with-matching-`intent_fingerprint` → return it;
different fingerprint → `IdempotencyConflict`; concurrent insert loser catches
`IntegrityError`, re-reads, returns the winner. Fingerprint = sha256(provider,
order_id, seat_id, staff_id, currency, expected_total, subtotal, tax, tip,
service_charge, discount, surcharge).

## Refund idempotency semantics
`create_refund_attempt(key=K)`: resolves+validates the charge attempt first, then
fingerprint = sha256(payment_id, charge_attempt_id, provider, amount_cents,
currency). Same key + same intent → existing; different → `IdempotencyConflict`;
concurrent loser re-reads (no raw IntegrityError). Linkage (#7): prefer deriving
the charge attempt from `payment_id`; enforce `payment_id` match, `status ==
SETTLED`, and `provider` match.

## Square charge error classification (#4)
- `SquareApiError` (definitive 4xx) → `FAILED`.
- `SquareTransportError` (timeout, dropped connection, **5xx**, 408/425/429) →
  `REQUIRES_RECONCILIATION` ("unknown submission outcome"). A timeout is never
  proof no charge exists.

## Square poll error classification (#9)
- `SquareTransportError` → stay `PROCESSOR_PENDING` (keep polling).
- Definitive `SquareApiError` (auth/config/not-found) → `REQUIRES_RECONCILIATION`
  (never PENDING forever).
- `COMPLETED` with no `payment_id` → `REQUIRES_RECONCILIATION` (not approval).

## Cancel state mapping (#11)
`CANCELED`→ok; `COMPLETED`→not ok + reconcile ("payment likely exists");
`PENDING`/`IN_PROGRESS`/`CANCEL_REQUESTED`→not ok + reconcile; unknown→reconcile;
any `SquareError`→not ok + reconcile. A 2xx alone never implies success.

## Currency source / verification (#5/#6)
Per-operation `currency` forwarded into `create_checkout`/`create_refund`.
`processor_currency` is read from the Square **Payment** `total_money.currency`
(authoritative), never `square.currency()`; None when unreadable.

## Processor amount / tip semantics (#17)
`expected_total_cents` is **pre-tip**. Terminal adds tip → processor captures
`base + tip`. `processor_amount_cents` = processor **pre-tip base**
(`captured_total − tip`), directly comparable to `expected_total_cents`;
`tip_cents` is the processor-confirmed tip. Both come from processor evidence and
are **write-once** (#8): a differing later value → `TransitionConflict`.

## Provider capability contract (#10/#19)
`capabilities: frozenset` advertised per provider; `register()` rejects any
capability whose backing method isn't implemented (`poll`/`lookup`/`authorize`/
`capture`/`handle_webhook`; REFUND/PARTIAL_REFUND ride the abstract `refund`).
`ManualProvider`={REFUND, PARTIAL_REFUND}; `SquareTerminalProvider`={POLLING,
REFUND, PARTIAL_REFUND, LOOKUP} (CAPTURE dropped — immediate-capture). Duplicate
registry keys rejected unless `override=True`. Auth/capture/webhook/lookup
contracts are defined + validated but **not yet consumed by settlement** (2c).

## Reconciliation authority (#13)
Only exit from `REQUIRES_RECONCILIATION`, always writing an `audit_event`:
- **Manual**: `actor` must be a Staff with OWNER/MANAGER role.
- **Automatic**: `automatic=True` requires `provider_evidence`; a note alone can
  never settle. Plain `transition()` out of reconciliation stays blocked.

## Tests run — commands, counts (0 failures)
Env: Python 3.14, SQLAlchemy 2.0.51, psycopg 3.3.4 (test-only), Postgres 16 @ :5433.
```
# SQLite (default)
test_config              PASS      test_payment_attempts    19 checks ok
test_refund_attempts     12 ok     test_payment_providers   43 checks ok
test_pg_concurrency      SKIP (no DSN)   test_pg_migration   SKIP (no DSN)
test_templates/security/admin/money/reconciliation/schedule  PASS

# PostgreSQL (PG_TEST_DSN set)
test_payment_attempts    PASS      test_refund_attempts     PASS
test_pg_concurrency      12 checks ok    test_pg_migration   12 checks ok
```
Skips: the two Postgres suites skip (exit 0) without `PG_TEST_DSN` — SQLite green
is never treated as concurrency/upgrade proof. Concurrency stability: **6/6**
consecutive full runs after this round.

### Migration-upgrade output (Postgres)
```
- test_clean_upgrade
    migration applied: [intent_fingerprint, processor_amount_cents,
      processor_currency, reconciled_at, reconciled_by, reconciliation_note,
      widened provider -> VARCHAR(30), backfilled provider square->square_terminal (3 rows)]
  ok provider_payment/checkout UNIQUE constraints exist after upgrade
  ok provider widened to 30 · new columns added · provider_refund_id dropped (all-null)
  ok all rows preserved · provider backfilled · financial identifiers preserved
- test_upgrade_blocks_on_duplicate_payment_id   ok (fails closed, no rewrite)
- test_upgrade_blocks_on_duplicate_checkout_id  ok (#15)
- test_duplicate_rejected_after_upgrade         ok (live constraint post-upgrade)
```

### Postgres concurrency output
```
- test_concurrent_same_key_create      one row, all callers same id, no errors
- test_concurrent_conflicting_transition  one winner, losers typed-refuse, status uncorrupted
- test_concurrent_refund_same_key      one RefundAttempt, all callers same (#16)
- test_provider_payment_id_unique      duplicate provider_payment_id rejected (#5)
- test_fk_enforced                     FK to missing order rejected
- test_one_settlement_per_attempt      one Payment backs one settled attempt
```

## Failure-injection matrix (proven now vs. deferred to 2c)
| Scenario | Now | By |
|---|---|---|
| Old PG schema upgrades; constraints added; data preserved; dups fail closed | ✅ | test_pg_migration |
| Concurrent same-key create/refund → one row | ✅ | test_pg_concurrency |
| Concurrent conflicting transitions → one winner | ✅ | test_pg_concurrency |
| Provider payment/checkout id uniqueness | ✅ | migration + concurrency |
| Charge transport ambiguity → reconcile (not FAILED) | ✅ | test_payment_providers |
| Poll transient vs definitive; cancel status mapping | ✅ | test_payment_providers |
| Processor currency from evidence; amount write-once | ✅ | providers + attempts |
| Reconciliation authority (role/evidence/audit) | ✅ | test_payment_attempts |
| **Processor success + DB failure mid-settlement; crash/restart recovery; duplicate poll settling twice** | ⛔ 2c/2d (live wiring + recovery worker) | — |

## Areas I am least confident about
1. **Real-data migration of `provider_refund_id`.** Drop path is proven for the
   all-NULL case; the "kept + reported" branch for non-NULL data is not exercised
   against a real populated legacy DB (none exists yet).
2. **`provider_evidence` is free text.** Automatic reconciliation requires it, but
   it isn't a structured/validated processor lookup result yet — the 2d recovery
   worker should pass a typed lookup, and the resolver should arguably re-verify.
3. **Currency: forward vs enforce.** I forward per-operation currency but do not
   yet *reject* a venue/provider single-currency mismatch before calling Square.
   If venues must be single-currency, that guard is still open (a 2c decision).
4. **Pre-tip base math** assumes Square's `total_money` includes tip so
   `base = total − tip`. Validated against synthetic payloads, not a live Square
   sandbox; worth confirming against real terminal data before 2c compares it.
5. **Capability validation proves presence, not correctness** — an override that
   internally `raise`s would still pass registration.
6. **SQLite constraint upgrade** creates a redundant unique index on an
   already-fresh SQLite DB (harmless); the SQLite `_constraint_exists` check is
   heuristic. Postgres (the production target) is exact.
...
@@ -192,8 +234,125 @@ def _run_postgres(engine: Engine) -> list[str]:
...
@@ -130,12 +157,19 @@ def create_checkout(
     note: str = "",
     allow_tip: bool = True,
...
@@ -149,9 +183,10 @@ def create_checkout(
     else:
...
@@ -179,6 +214,39 @@ def get_payment(payment_id: str) -> dict:
...
@@ -195,6 +263,43 @@ def wait_for_checkout(checkout_id: str, timeout_s: float = 120.0, interval_s: fl
     return checkout
