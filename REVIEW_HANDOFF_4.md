# Review Handoff #4 — Handoff-3 Deep-Review Blockers Resolved

**For:** external reviewer (ChatGPT). **Stage 2c is NOT started** (as instructed).
Covers only `CLAUDE_HANDOFF3_DEEP_REVIEW_FEEDBACK.md` findings #1–#8, applied
additively before any wiring into live charge/refund routes.

## Commit range
- **main (pre-remediation) SHA:** `07ee4f1af26aa383b7de06377f18caa98b153e4b`
- **Handoff #3 HEAD:** `0621b6ae5207ef5f9bc0081cb5caa8cfb188c94f`
- **HEAD (this handoff):** `9dfe1d7f3fe7cc60b2c1d88c9d2414995ad6a1a6`
- Branch: `fix/p0-security-and-payments`
- **Commits added this round (0621b6a..HEAD):**
  - `f99e815` A — drop legacy provider default, backfill terminal instrument, fail-closed refund_id
  - `5572a16` B — never approve a charge on incomplete processor evidence
  - `58abfd1` C — derive legacy refund provider + enforce refund currency
  - `9dfe1d7` D — reject unknown capabilities; classify Square 4xx before FAILED

## `git status --short`
No uncommitted **source** changes. Untracked non-source: `REVIEW_HANDOFF*.md`,
`BUSINESS_STRATEGY*.md` / `COMMERCIAL_STRATEGY*` / `MISE_*.pptx` / `.codex_pptx_build/`
(business docs), `.github/`, `restaurant-app-review.zip`.

## Finding-by-finding
| # | Sev | Finding | Status | Where |
|---|---|---|---|---|
| 1 | CRIT | Drop legacy `payment_attempt.provider` default | ✅ | `migrate._migrate_payment_hardening` step 1c |
| 2 | CRIT | Backfill `card_terminal` instrument → square_terminal | ✅ | `migrate` step 1b |
| 3 | CRIT | No PROCESSOR_APPROVED without complete evidence | ✅ | `SquareTerminalProvider.poll` |
| 4 | CRIT | Legacy refund provider from Payment/instrument | ✅ | `refund_attempts._resolve_charge_attempt` |
| 5 | CRIT | Refund currency matches original charge | ✅ | `refund_attempts._validate_refund_currency` |
| 6 | HIGH | Non-null legacy `provider_refund_id` fail-closed | ✅ | `migrate` step 2 (strict → MigrationError) |
| 7 | HIGH | Reject unknown capability names | ✅ | `payment_providers.ALL_CAPABILITIES` |
| 8 | HIGH | Classify Square 4xx before FAILED | ✅ | `square.classify_charge_error`; `charge()` |

## Exact migration operations added (`_migrate_payment_hardening`)
1. **1b — instrument backfill:** `UPDATE payment_instrument SET provider='square_terminal' WHERE code='card_terminal' AND provider <> 'square_terminal'`.
2. **1c — drop legacy default (Postgres):** guarded on `information_schema.columns.column_default IS NOT NULL`, then `ALTER TABLE payment_attempt ALTER COLUMN provider DROP DEFAULT`.
3. **provider_refund_id retirement:** all-NULL → `DROP COLUMN [IF EXISTS]`; **non-NULL under `strict=True` → raise `MigrationError`** (fail closed); non-strict → keep + report.
(Existing from prior round, still applied: provider backfill `square→square_terminal`, widen `provider`→VARCHAR(30), additive columns, provider-scoped UNIQUE with duplicate-scan fail-closed.)

## Introspected provider default after PostgreSQL upgrade
```
before upgrade: payment_attempt.provider column_default = 'square'::character varying
after  upgrade: payment_attempt.provider column_default = NULL      ✔ (#1)
```

## PaymentInstrument provider backfill behavior
```
legacy card_terminal (no provider col) → provider = 'square_terminal'   ✔ (#2)
legacy cash          (no provider col) → provider = 'manual'            ✔ (#2)
```

## Legacy provider_refund_id behavior
```
all-NULL column      → dropped
non-NULL + strict     → MigrationError "…provider_refund_id still holds N non-null row(s)…"
non-NULL + non-strict → kept + reported (dev only)
```

## Square evidence completeness rules (poll → approval)
`COMPLETED` maps to `PROCESSOR_APPROVED` **only** when all present: authoritative
`provider_payment_id`, `processor_amount_cents` (pre-tip base), and
`processor_currency` (from the Payment object). Any missing piece — including a
payment-lookup transport OR definitive failure — → `REQUIRES_RECONCILIATION`
(carrying the payment id).

## Legacy refund provider derivation
Attempt-backed: validated against the charge attempt (payment match, SETTLED,
provider match). Legacy (no attempt): provider is derived from
`Payment.instrument.provider`; a caller-supplied provider that disagrees is
rejected. The caller is never trusted to classify a historical payment.

## Refund currency rules
Attempt-backed: `refund.currency == charge_attempt.currency` and, when set,
`== charge_attempt.processor_currency`. Legacy: `refund.currency ==
config.venue_currency()` (`VENUE_CURRENCY` → `SQUARE_CURRENCY` → `CAD`) — the
authoritative fallback since `Payment` has no per-row currency column (see least-
confidence note 1).

## Square 4xx classification (`classify_charge_error`)
`SquareApiError` carries `status_code`, Square `code`, and `error_category`.
- **DECLINE** (402 / `PAYMENT_METHOD_ERROR` / decline codes) → `FAILED`.
- **CONFLICT** (409 / `IDEMPOTENCY_KEY_REUSED`) → `REQUIRES_RECONCILIATION` (charge may exist).
- **AUTH_CONFIG** (401/403 / `AUTHENTICATION_ERROR`) → `REQUIRES_RECONCILIATION`.
- **INVALID_TARGET** (404 / device/location) → `REQUIRES_RECONCILIATION`.
- **UNEXPECTED** (any other 4xx) → `REQUIRES_RECONCILIATION`.
A new charge is never inferred safe merely because the status was 4xx.

## Capability vocabulary validation
`ALL_CAPABILITIES` is the closed set. `register()` rejects any advertised
capability outside it (typo/unknown → `ValueError`) before the method-backing
check (#10 from the prior round).

## Tests run — commands, counts (0 failures)
Env: Python 3.14, SQLAlchemy 2.0.51, psycopg 3.3.4 (test-only), Postgres 16 @ :5433.
```
# SQLite (default)
test_payment_attempts   19 ok   test_refund_attempts   18 ok
test_payment_providers  55 ok   test_config PASS
...
test_pg_migration       17 ok   test_pg_concurrency    12 ok
test_payment_attempts / test_refund_attempts  PASS
```
Skips: the two PG suites skip (exit 0) without `PG_TEST_DSN`. Concurrency
stability: **6/6** consecutive full runs.

### PostgreSQL migration-upgrade output
```
- test_clean_upgrade                     ok (constraints, widen, backfill, drop refund_id, data preserved)
- test_provider_default_removed          ok  provider default = NULL after upgrade (#1)
- test_card_terminal_instrument_backfilled ok  card_terminal→square_terminal; cash stays manual (#2)
- test_nonnull_provider_refund_id_fails_strict ok  strict MigrationError on non-null legacy data (#6)
- test_upgrade_blocks_on_duplicate_payment_id / _checkout_id  ok (fail closed)
- test_duplicate_rejected_after_upgrade  ok (live constraint)
```

### PostgreSQL concurrency output
```
- test_concurrent_same_key_create        one row, all callers same id
...
- test_provider_payment_id_unique / test_fk_enforced / test_one_settlement_per_attempt  ok
```

## Areas I am least confident about
1. **Legacy refund currency has a weak source.** `Payment` has no per-row
   currency column, so legacy validation falls back to `venue_currency()` (a
   single-currency assumption). Attempt-backed refunds are strong; legacy is only
   as good as the venue default. A real multi-currency history would need a
   currency column on `Payment`.
2. **`provider_refund_id` auto-migration not implemented.** The non-null case
   fails closed (raises) rather than converting rows into `RefundAttempt`; that
   conversion is left for an operator/2d, and is unexercised against real data.
3. **Square 4xx classification depends on Square's `code`/`category` strings.** The
   decline-code set and category names are from Square's documented taxonomy but
   not validated against a live sandbox; an unrecognized code falls through to
   UNEXPECTED → reconciliation (safe default), but a real decline miscoded by
   Square could over-reconcile rather than FAIL.
4. **`processor_amount` pre-tip base math** still assumes Square `total_money`
   includes tip; validated on synthetic payloads only.
5. **Capability validation proves presence, not correctness** (unchanged) — an
   override that internally raises would still register.
...
@@ -192,8 +234,154 @@ def _run_postgres(engine: Engine) -> list[str]:
...
@@ -195,6 +306,43 @@ def wait_for_checkout(checkout_id: str, timeout_s: float = 120.0, interval_s: fl
     return checkout
