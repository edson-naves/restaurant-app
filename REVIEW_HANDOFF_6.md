# Review Handoff #6 — Handoff-5 Blockers Resolved

**For:** external reviewer (ChatGPT). **Stage 2c is NOT started** (as instructed).
Covers only `CLAUDE_HANDOFF5_DEEP_REVIEW_FEEDBACK.md` findings #1–#4, applied
...
- **Handoff #5 HEAD:** `ceb2004aad28cb26b89255c0d1323de77c9e3448`
- **HEAD (this handoff):** `f44bb3f66562b6621cd6196996a0a695e46b5a2e`
- Branch: `fix/p0-security-and-payments`
- **Commits added this round (ceb2004..HEAD):**
  - `3b6f7bd` gate refund reconciliation behind an authored resolver (#1)
  - `cfac2bf` enforce external-approval evidence at the state-machine boundary (#2)
  - `9c2e954` unsupported cancel fails explicitly; add CANCEL capability (#3)
  - `f44bb3f` parse processor evidence defensively (untrusted payload) (#4)
...
|---|---|---|---|---|
| 1 | CRIT | Refund reconciliation gated by an authored resolver | ✅ | `refund_attempts.resolve_refund_reconciliation` |
| 2 | CRIT | External approval evidence enforced at state machine | ✅ | `payment_attempts._require_approval_evidence` |
| 3 | HIGH | Unsupported cancel fails explicitly; CANCEL capability | ✅ | `payment_providers` (base `cancel` raises) |
| 4 | HIGH | Processor evidence parsed defensively | ✅ | `square._safe_int` / `completed_payment_evidence` |

## Refund reconciliation resolver contract (#1)
`transition_refund()` refuses to leave `REQUIRES_RECONCILIATION` (raises unless
`_from_resolver`). The **only** exit is `resolve_refund_reconciliation(db, refund,
*, resolved_status, note, actor=None, automatic=False, provider_evidence=None,
provider_refund_id=None)` — a mirror of the charge-attempt resolver.

### Authority / evidence / audit behavior
- **Manual:** `actor` must be a Staff with an OWNER/MANAGER role, else
  `ReconciliationAuthorityError`.
- **Automatic:** `automatic=True` requires non-empty `provider_evidence`; a note
  alone can never resolve.
- Always: requires a note, writes an `audit_event` (`action="reconcile_refund_
  attempt"`, actor id or NULL for system), persists `reconciled_at/by/note`, and
  routes through the CAS `transition_refund(..., _from_resolver=True)` so
  concurrent resolutions have exactly one winner.

## External approval invariant (#2)
`transition()` enforces, at the state-machine boundary, that entering
`PROCESSOR_APPROVED` for an **external** provider (`get_provider(attempt.provider)
.is_external`) requires effective (persisted OR supplied) `provider_payment_id`,
a non-negative `processor_amount_cents`, and a structurally valid
`processor_currency`. Manual/local providers approve instantly with no external
evidence. This no longer relies on the Square adapter alone.

## Cancel capability / unsupported-cancel behavior (#3)
Base `PaymentProvider.cancel()` now **raises `NotImplementedError`** instead of
returning `CancelResult(ok=True)`. `Capability.CANCEL` added to the closed
vocabulary and the method map; `register()` rejects a provider that advertises
CANCEL without overriding `cancel()`. `SquareTerminalProvider` advertises CANCEL
(already implemented); `ManualProvider` does not (its cancel raises — explicit,
never false success).

## Processor parse-failure behavior (#4)
`completed_payment_evidence()` treats the Square Payment as untrusted:
- amounts via `_safe_int` → `None` on anything non-numeric;
- a present-but-malformed tip → `None` (not silently `0`);
- currency normalized to uppercase; blank/absent → `None`;
- the whole parse is wrapped — any malformed payload → all-`None` evidence.
`_evidence_coherent()` rejects a `None` tip too. So malformed numbers/objects map
to `REQUIRES_RECONCILIATION` (never a crash); a lowercase valid currency is
...
```
# SQLite (default)
test_payment_attempts   26 ok   test_refund_attempts   28 ok
test_payment_providers  70 ok   test_config PASS
...
- Refund reconciliation: bare→COMPLETED/FAILED rejected; unauthorized rejected;
  automatic-without-evidence rejected; manager+audit accepted; automatic+evidence
  accepted; **Postgres concurrent resolution → one winner**.
- External approval: payment-id-only / missing-amount / missing-currency /
  missing-payment-id rejected; complete evidence approves; manual approval valid.
- Cancel: manual cancel raises; advertising CANCEL without implementing rejected;
  Square advertises+implements.
- Evidence parsing: non-numeric total/tip, unexpected numeric type, malformed
  payment object → reconcile; lowercase currency normalized → approve.

### PostgreSQL migration + concurrency output (unchanged, still green)
```
...
2. **`provider_refund_id` auto-migration** still not implemented (non-null fails
   closed inside the atomic tx).
3. **Approval evidence vs. amount-match:** `_require_approval_evidence` checks
   presence + structure + non-negativity, but does not yet compare
   `processor_amount_cents` to `expected_total_cents` — that amount-reconciliation
   decision is Stage 2c settlement's job.
4. **Square code/category taxonomy** for 4xx classification is documented, not
   sandbox-verified; unknown → UNEXPECTED → reconcile (safe).
5. **Capability validation proves presence, not correctness** — an override that
   internally raises still registers.
...
@@ -195,6 +306,58 @@ def wait_for_checkout(checkout_id: str, timeout_s: float = 120.0, interval_s: fl
     return checkout
