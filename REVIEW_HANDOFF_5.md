# Review Handoff #5 — Handoff-4 Blockers Resolved

**For:** external reviewer (ChatGPT). **Stage 2c is NOT started** (as instructed).
Covers only `CLAUDE_HANDOFF4_DEEP_REVIEW_FEEDBACK.md` findings #1–#5, applied
...
- **Handoff #4 HEAD:** `9dfe1d7f3fe7cc60b2c1d88c9d2414995ad6a1a6`
- **HEAD (this handoff):** `ceb2004aad28cb26b89255c0d1323de77c9e3448`
- Branch: `fix/p0-security-and-payments`
- **Commits added this round (9dfe1d7..HEAD):**
  - `646aabc` narrow terminal backfill + atomic payment-hardening migration (#1, #2)
  - `911b61f` financial currency defaults to venue_currency(), not CAD (#3)
  - `b8b0636` validate processor evidence coherence before approval (#4)
  - `ceb2004` legacy refund provider must be derivable, else fail closed (#5)
...
|---|---|---|---|---|
| 1 | CRIT | Terminal-instrument backfill non-destructive to future providers | ✅ | `migrate` step 1b (`WHERE provider='manual'`) |
| 2 | HIGH | Payment-hardening migration atomic | ✅ | `_migrate_payment_hardening` (single tx) |
| 3 | HIGH | No hard-coded CAD at financial boundaries | ✅ | `create_attempt`/`create_refund_attempt` → `venue_currency()` |
| 4 | HIGH | Processor evidence semantically validated | ✅ | `payment_providers._evidence_coherent` |
| 5 | MED | Legacy provider fail-closed when underivable | ✅ | `refund_attempts._resolve_charge_attempt` |

## Exact revised terminal-instrument migration condition (#1)
```sql
UPDATE payment_instrument SET provider='square_terminal'
WHERE code='card_terminal' AND provider='manual';
```
Narrowed from `provider <> 'square_terminal'` to `provider = 'manual'` — it now
repairs *only* the row that took the provider-column's `'manual'` default. A
deliberately-chosen provider (e.g. `stripe_terminal`) is never overwritten, and a
re-run matches nothing (idempotent).

## Migration atomicity / resumability decision (#2)
**Chosen: atomic (single transaction).** `_migrate_payment_hardening` runs all
steps — provider backfill, terminal-instrument backfill, drop legacy default,
retire `provider_refund_id`, add provider-scoped UNIQUE constraints — inside one
`engine.begin()`. Postgres has transactional DDL, so a failure at any step
(duplicate constraint, non-null legacy `provider_refund_id` under strict) rolls
back **every** earlier step; the DB is never left partially hardened. The prior
additive column-adds (ADDED/WIDENED) remain individually idempotent/resumable.

### Failure-injection migration test output (Postgres)
```
- test_hardening_atomic_rollback
  ok  strict migration fails on the duplicate
  ok  earlier steps rolled back: provider default still present (atomic — #2)
  ok  provider backfill rolled back too (rows still 'square')
- test_hardening_is_idempotent
  ok  second migration run is a no-op ([])
  ok  provider values stable across repeated runs
- test_card_terminal_preserves_explicit_provider
  ok  card_terminal + alternate provider is left unchanged (#1)
  ok  card_terminal + square_terminal unchanged (#1)
```

## Currency service contract (#3)
`create_attempt(currency: str | None = None)` and
`create_refund_attempt(currency: str | None = None)` normalize an omitted value
with `config.venue_currency()` (`VENUE_CURRENCY` → `SQUARE_CURRENCY` → `CAD`),
upper-cased. No hard-coded `'CAD'` at the financial service boundary. Proven with
`VENUE_CURRENCY=USD`: an omitted charge/refund currency resolves to `USD`.

## Processor evidence validation rules (#4)
Before `PROCESSOR_APPROVED`, `_evidence_coherent(ev)` requires:
`captured_total_cents` and `base_cents` present and `>= 0`; `tip_cents >= 0` and
`<= captured_total`; a structurally valid 3-letter alpha `currency`. Any failure —
or a missing payment id / lookup failure (from the prior round) — →
`REQUIRES_RECONCILIATION`. (`base = captured_total − tip`, comparable to the
pre-tip `expected_total_cents`.)

## Legacy-provider fail-closed rules (#5)
For a legacy Payment (no PaymentAttempt), the refund provider is derived from
`Payment.instrument.provider`. It now **fails closed** when it cannot be derived:
missing instrument, blank provider, or an *unregistered* provider all raise —
caller input never becomes authoritative. Only a derived, registered provider
matching the caller is accepted.

## Tests run — commands, counts (0 failures)
Env: Python 3.14, SQLAlchemy 2.0.51, psycopg 3.3.4 (test-only), Postgres 16 @ :5433.
```
# SQLite (default)
test_payment_attempts   20 ok   test_refund_attempts   21 ok
test_payment_providers  61 ok   test_config PASS
...
test_pg_migration       25 ok   test_pg_concurrency    12 ok
...
### PostgreSQL migration output (highlights)
```
clean upgrade: constraints added, provider widened, backfill square->square_terminal,
  provider_refund_id dropped (all-null), data preserved, default = NULL post-upgrade
card_terminal: manual->square_terminal; cash stays manual; explicit provider preserved
idempotent re-run: no-op; atomic rollback on injected duplicate; non-null refund_id fails strict
duplicate payment_id/checkout_id: fail closed; live constraint rejects post-upgrade
```

### PostgreSQL concurrency output
```
concurrent same-key create -> one row; conflicting transition -> one winner, typed refusals;
concurrent same-key refund -> one RefundAttempt; provider payment-id uniqueness;
FK enforced; one settlement per attempt. (6/6 stable)
```

## Areas I am least confident about
1. **Legacy refund currency source** (unchanged from #4-round): `Payment` has no
   per-row currency column, so legacy currency validation falls back to
   `venue_currency()`. Strong for attempt-backed refunds; single-currency
   assumption for legacy.
2. **`provider_refund_id` auto-migration** still not implemented — non-null fails
   closed (now inside the atomic tx, so it rolls back cleanly); conversion into
   `RefundAttempt` is left for an operator/2d, unexercised against real data.
3. **Evidence coherence is structural/arithmetic, not cross-checked against
   `expected_total`.** It rejects impossible values but does not yet compare the
   processor base to the attempt's expected total — that amount-match decision is
   Stage 2c settlement's job.
4. **Square 4xx classification** depends on Square's documented code/category
   strings; unrecognized codes fall to UNEXPECTED → reconciliation (safe).
5. **Atomic DDL is Postgres-specific.** On SQLite the hardening also runs in one
   `engine.begin()`, but SQLite's DDL-transaction guarantees are weaker; the
   production target (Postgres) is the one that matters and is proven.
